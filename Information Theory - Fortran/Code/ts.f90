MODULE TS
    ...
    !annealing for the traveling salesman NP-problem.
    !All the necessary subroutines are then collected
    !in the subroutine called "TravelingSalesman" that 
...
IMPLICIT NONE 
    ...
    real*8, dimension(:,:), allocatable :: ADJ !-> adjency matrix
    
    !QUANTUM ANNEALING PARAMETERS
    type(hamiltonian) :: hamilt !-> quantum hamiltonian
    real*8 :: dtime=0.0001, h0param=1d0
    integer, dimension(:), allocatable :: spinQA, Order
...
SUBROUTINE InitializationParameters()
    ...
    !problem from the command line: number of nodes/cities C, number 
    !of edges E, coefficients A and B of the hamiltonian.
    !The number of spins is set to N=C*C.
    !The directory in which save the results is created and then
    !the adjency matrix is built.
    
    !OBJECT TO BUILD THE PATH
    character(len=100) :: Astr, Bstr, Cstr, Estr

    !READ TE MAIN PARAMETERS FROM THE COMMAND LINE
    print*, ''
    print *, 'Number of cities:'
    read(*,*) C
    print *, 'Maximum number of edges:', int(C*(C-1)*0.5)
    ...
    write(Astr,*) int(A)
    write(Bstr,*) int(B)
    path=trim('TS/C'//trim(adjustl(Cstr))//'E'// &
                trim(adjustl(Estr))//'A'//       &
                trim(adjustl(Astr))//'B'//       &
                trim(adjustl(Bstr))//'/')
    ...
    call ProblemInitialization(C, E, ADJ, &
...
SUBROUTINE TuningParameters(Verbose, Analysis)
    ...
    do ii=1,size(Ttuning)
        call Metropolis(C, ADJ, A, B, Ttuning(ii), Repeat, &
    ...
    do ii=1,size(Ttuning)
        do jj=1,size(Mtuning)
            call QuantumMetropolis(C,Mtuning(jj),ADJ,A,B,     &
                                Ttuning(ii), Repeat,          &
...
SUBROUTINE GSSQA()
    ...
    do ii=1,Repeat
        call QuantumMetropolis(C, M, ADJ, A, B, TemperatureSQA,  &
                                1, 10000, Energy(ii),            &
                                'None', 'None', .False.,.False.)
    end do
    !min energy
...
SUBROUTINE QAResolution()
    ...
    call QuantumH(C, ADJ, A, B, hamilt%hP, &
    ...
    call AQCAlgorithm(N, hamilt, MaxTime(size(MaxTime)), dtime,    &
                        h0param,spinQA, trim(path)//'QAenergy.txt',&
                        trim(path)//'QAprob.txt',                  &
    ...
    do ii=1,C 
        do jj=0,C-1 
            if (spinQA(ii+jj*C) > 0) then
                Order(ii) = jj+1
                exit
            end if
        end do
    end do
    call Save(Order, trim(path)//'QAspin.txt')
    deallocate(spinQA,Order)
...
SUBROUTINE SAResolution()
    ...
    do ii=1,size(MaxTime)
        call Metropolis(C, ADJ, A, B, TemperatureSA, Repeat, &
                            MaxTime(ii), ComputedE0,         &
                            trim(path)//'SAenergy.txt',      &
                            trim(path)//'SAspin.txt',        &
                            .True.,debts)
...
SUBROUTINE Performance()
    ...
    !the resultion of the traveling salesman. The three methods are 
    !applied for different values of nodes/cities (with maximum number
    !of edges) and the cpu time needed to perform the computation and 
    ...
    !allocate the nodes
    allocate(nodes(7))
    nodes = (/3,4,5,6,8,10,12/)

    !allocate time and res energy
    allocate(sa(size(nodes),3),sqa(size(nodes),3),aqc(1,3))
    sa = 0
    sqa = 0
    aqc = 0
    ...
    do nn=1,size(nodes)
        ...
        call debug(debts,'Node', nodes(nn))
        call debug(debts,'-----------------')

        !edges
        edges = int(nodes(nn)*(nodes(nn)-1)*0.5)
        C=nodes(nn)
        N=nodes(nn)**2
        E=edges
        A=2d0
        ...
        !initialize the problem 
        call ProblemInitialization(C,E,ADJ,'None', &
                                    .False.)
        ...
        call debug(debts, 'Temperature for SA', TemperatureSA)
        ...
        call debug(debts, 'Temperature for SQA', TemperatureSQA)
        call debug(debts, 'M for SQA', M)
        ...
        if (N<=12) then
            ...
            call debug(debts,'AQC energy',hamilt%E_gs)
        else
            call GSSQA()
        end if
        ...
        call cpu_time(tstart)
        call Metropolis(C, ADJ, A, B, TemperatureSA, &
        ...
        call debug(debts,'SA energy',ComputedE0)

        !SIMULATED QUANTUM ANNEALING
        !start time computation
        call cpu_time(tstart)
        call QuantumMetropolis(C, M, ADJ, A, B,      &
                                TemperatureSQA, 100, &
                                10000, ComputedE0,   &
                                'None','None',       &
                                .False., .False.)
        ...
        call debug(debts,'SQA energy',ComputedE0)
        
        !deallocate adjency matrix
        deallocate(ADJ)  
    end do

    call Save(sa, 'TS/SAperf.txt')
    call Save(sqa, 'TS/SQAperf.txt')
    call Save(aqc, 'TS/QAperf.txt')
